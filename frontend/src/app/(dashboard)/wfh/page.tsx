  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
          ...
          <div className="bg-[#1e1e2e] border border-red-500/40 rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <div className="bg-white/5 rounded-xl p-3 mb-5 space-y-1.5">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-300">
                ...
                Full Day / Morning — apply before <span className="font-bold text-white ml-1">9:45 AM</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Work From Home</h1>
          <p className="text-slate-400 text-sm">Submit and track your remote work requests</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {step === 1 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-white">Choose WFH Type</h2>
                <p className="text-slate-400 text-sm mt-1">Select the session you want to work from home</p>
              </div>
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-white">Select Date{isHalfDay ? "" : " Range"}</h2>
              ...
              <div className="flex items-center gap-3 p-3.5 bg-white/5 border border-white/10 rounded-xl mb-6">
                <span className="text-2xl">{chosenOpt.icon}</span>
                <div>
                  <p className="text-sm font-semibold text-white">{chosenOpt.label}</p>
                  <p className="text-xs text-slate-400">{chosenOpt.desc}</p>
                </div>
              </div>
              ...
              {isHalfDay ? (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-800 border border-white/10 text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-amber-500 transition-colors"
                  />
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-white">Describe Your Tasks</h2>
                <p className="text-slate-400 text-sm mt-1">Tell your manager what you'll be working on</p>
              </div>
              <textarea
                rows={6}
                {...form.register("reason")}
                placeholder="e.g. Working on the Q3 report, attending online client calls, fixing critical bugs on the production server…"
                className="w-full bg-slate-800 border border-white/10 text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-amber-500 placeholder:text-slate-600 resize-none transition-colors"
              />
          ...
          {step === 4 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                ...
                <p className="text-slate-400 text-sm mt-1">Confirm your WFH request details before submitting</p>
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-xl">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{chosenOpt.icon}</span>
                    <div>
                      <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">WFH Type</p>
                      <p className="text-sm font-semibold text-white mt-0.5">{chosenOpt.label}</p>
                    </div>
                  </div>
                ...
                <div className="p-4 bg-white/5 border border-white/10 rounded-xl">
                  <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-2">Reason / Tasks</p>
          ...
          <div className={`flex mt-8 gap-3 ${step === 1 ? "justify-end" : "justify-between"}`}>
            {step > 1 && (
              <button
                type="button"
                onClick={prevStep}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/15 text-slate-300 text-sm font-semibold hover:bg-white/5 transition-colors"
              >
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-0.5">All your remote work requests and their approval status</p>
          </div>
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-white/10 text-slate-300">{requests.length} requests</span>
        </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      {(user?.role === "Super Admin" || user?.role === "Team Lead") && req.user && (
                        <p className="text-xs font-bold text-slate-400 mb-1.5">{req.user.first_name} {req.user.last_name}</p>
                      )}
