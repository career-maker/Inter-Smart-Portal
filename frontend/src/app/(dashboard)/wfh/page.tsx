...
export default function WfhPage() {
  ...
  const [showApplyForm, setShowApplyForm]       = useState(!isSuperAdmin);
  ...
  return (
    <div className="space-y-7 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-base md:text-lg font-bold text-slate-900 dark:text-white">
              {isSuperAdmin ? (
                ...
                pendingCount === 0 ? "No pending WFH requests in your team" : (
                  <>You have <span className="text-amber-600 dark:text-amber-400 font-extrabold mx-1">{pendingCount}</span> pending WFH request{pendingCount !== 1 ? "s" : ""}</>
                )
              ) : (
            ...
            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              {isSuperAdmin
                ? "Review and monitor company-wide remote work requests and their status."
                : "Submit and track your remote work sessions and manager approvals."
              }
            </p>
          </div>
        </div>
        ...
        {!isSuperAdmin && (
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button
              ...
              className={`w-full sm:w-auto px-5 py-2.5 font-semibold text-xs rounded-xl shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5 ${
                showApplyForm
      ...
      {/* ── Multi-Step Apply Form (For Non-Admin when toggled) ─────── */}
      {!isSuperAdmin && showApplyForm && (
      ...
      {/* ── All WFH Requests Table Section ────────────────────────── */}
        ...
        <div className="px-6 py-4 border-b border-slate-200/90 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-850">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white">
              {isSuperAdmin ? "All WFH Requests" : "My WFH Requests"}
            </h2>
            <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">
              {isSuperAdmin
                ? "Complete list of remote work requests across the company"
                : "History of your remote work requests and their approval status"
              }
            </p>
          </div>
