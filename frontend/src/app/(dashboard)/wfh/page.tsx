...
function DurationBadge({ type }: { type: string }) {
  ...
  const m = map[type] ?? { cls: "bg-white/10 text-muted-foreground", label: type };
            ...
            <div className="flex flex-col items-center gap-1.5">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all duration-300 ${
                ...
                active ? "bg-amber-500/20 border-amber-500 text-amber-300" :
                         "bg-white/5 border-border text-slate-500"
              }`}>
                {done ? <CheckCircle className="w-4 h-4" /> : s.id}
              </div>
              <div className="text-center hidden sm:block">
                <p className={`text-[11px] font-bold leading-tight ${active ? "text-amber-300" : done ? "text-muted-foreground" : "text-slate-600"}`}>{s.title}</p>
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1e1e2e] border border-red-500/40 rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-foreground">Application Not Allowed</h3>
            ...
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 mb-5">
              {wfhWarning.split("\n\n").map((line, i) => (
                <p key={i} className={`text-sm ${i === 0 ? "font-bold text-red-300 mb-2" : "text-muted-foreground"}`}>{line}</p>
              ))}
            </div>
            <div className="bg-white/5 rounded-xl p-3 mb-5 space-y-1.5">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                Full Day / Morning — apply before <span className="font-bold text-foreground ml-1">9:45 AM</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <span className="w-2 h-2 rounded-full bg-violet-400 flex-shrink-0" />
                Afternoon Session — apply before <span className="font-bold text-foreground ml-1">2:30 PM</span>
              </div>
            </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Work From Home</h1>
          <p className="text-muted-foreground text-sm">Submit and track your remote work requests</p>
        </div>
      </div>

      {/* ── Step Form Card ────────────────────────────────────────── */}
      <div className="bg-white/5 border border-border rounded-2xl p-6 md:p-8 backdrop-blur-md">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {step === 1 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-foreground">Choose WFH Type</h2>
                <p className="text-muted-foreground text-sm mt-1">Select the session you want to work from home</p>
              </div>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-5 rounded-xl border-2 transition-all text-left group ${
                      selected
                        ...
                        : "border-border hover:border-white/25 hover:bg-white/5"
                    }`}
                  >
                    ...
                    {selected
                      ...
                      : <ChevronRight className="w-5 h-5 text-slate-600 shrink-0 group-hover:text-muted-foreground transition-colors" />
                    }
                  </button>
                );
              })}
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-foreground">Select Date{isHalfDay ? "" : " Range"}</h2>
              ...
              <div className="flex items-center gap-3 p-3.5 bg-white/5 border border-border rounded-xl mb-6">
                <span className="text-2xl">{chosenOpt.icon}</span>
                <div>
                  <p className="text-sm font-semibold text-foreground">{chosenOpt.label}</p>
                  <p className="text-xs text-muted-foreground">{chosenOpt.desc}</p>
                </div>
              </div>
              ...
              {isHalfDay ? (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-card border border-border text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-amber-500 transition-colors"
                  />
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                <h2 className="text-lg font-bold text-foreground">Describe Your Tasks</h2>
                <p className="text-muted-foreground text-sm mt-1">Tell your manager what you'll be working on</p>
              </div>
              <textarea
                rows={6}
                {...form.register("reason")}
                placeholder="e.g. Working on the Q3 report, attending online client calls, fixing critical bugs on the production server…"
                className="w-full bg-card border border-border text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-amber-500 placeholder:text-slate-600 resize-none transition-colors"
              />
          ...
          {step === 4 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="mb-6">
                ...
                <p className="text-muted-foreground text-sm mt-1">Confirm your WFH request details before submitting</p>
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex items-center justify-between p-4 bg-white/5 border border-border rounded-xl">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{chosenOpt.icon}</span>
                    <div>
                      <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">WFH Type</p>
                      <p className="text-sm font-semibold text-foreground mt-0.5">{chosenOpt.label}</p>
                    </div>
                  </div>
                ...
                <div className="p-4 bg-white/5 border border-border rounded-xl">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold mb-2">Reason / Tasks</p>
          ...
          <div className={`flex mt-8 gap-3 ${step === 1 ? "justify-end" : "justify-between"}`}>
            {step > 1 && (
              <button
                type="button"
                onClick={prevStep}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/15 text-muted-foreground text-sm font-semibold hover:bg-white/5 transition-colors"
              >
      ...
      <div className="bg-white/5 border border-border rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-foreground">WFH History</h2>
            <p className="text-muted-foreground text-xs mt-0.5">All your remote work requests and their approval status</p>
          </div>
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-white/10 text-muted-foreground">{requests.length} requests</span>
        </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      {(user?.role === "Super Admin" || user?.role === "Team Lead") && req.user && (
                        <p className="text-xs font-bold text-muted-foreground mb-1.5">{req.user.first_name} {req.user.last_name}</p>
                      )}
                      ...
                      <p className="text-sm text-muted-foreground truncate pr-4">{req.reason}</p>
