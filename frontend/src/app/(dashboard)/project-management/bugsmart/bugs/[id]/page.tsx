...
export default function BugzillaBugDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [attachmentUrlTitle, setAttachmentUrlTitle] = useState("");
  const [attachmentDescription, setAttachmentDescription] = useState("");
  const [addingAttachmentUrl, setAddingAttachmentUrl] = useState(false);
  ...
  const handleAddUrlAttachment = async (e: React.FormEvent) => {
    ...
    setAddingAttachmentUrl(true);
    try {
      const res = await bugzillaApi.addAttachmentUrl(
        bugId,
        formattedUrl,
        attachmentUrlTitle.trim() || undefined,
        attachmentDescription.trim() || undefined
      );
      setAttachments((prev) => [res.data, ...prev]);
      setAttachmentUrl("");
      setAttachmentUrlTitle("");
      setAttachmentDescription("");
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.message || "Failed to add URL attachment.");
    } finally {
      setAddingAttachmentUrl(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {activeTab === "attachments" && (
            <div className="space-y-4">
              {(canReport || canDevelop) && (
                <form onSubmit={handleAddUrlAttachment} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 shadow-xs">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5">
                    <input
                      type="url"
                      placeholder="Paste link (Loom, Google Drive, Figma, screenshot URL)…"
                      value={attachmentUrl}
                      onChange={(e) => setAttachmentUrl(e.target.value)}
                      required
                      className="sm:col-span-6 px-3 py-2 text-xs rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-rose-500"
                    />
                    <input
                      type="text"
                      placeholder="Title (e.g. Screen recording)"
                      value={attachmentUrlTitle}
                      onChange={(e) => setAttachmentUrlTitle(e.target.value)}
                      className="sm:col-span-3 px-3 py-2 text-xs rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-rose-500"
                    />
                    <button
                      type="submit"
                      disabled={addingAttachmentUrl || !attachmentUrl.trim()}
                      className="sm:col-span-3 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 disabled:opacity-50 transition-colors inline-flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      {addingAttachmentUrl ? "Adding…" : "Add URL"}
                    </button>
                  </div>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-1.5 text-xs rounded-xl bg-slate-50/60 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 text-slate-900 dark:text-white focus:outline-none"
                  />
                </form>
              )}
              ...
              <div className="space-y-2">
                {attachments.length === 0 ? (
                  <div className="p-8 text-center text-xs text-slate-400 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    No attachments or URLs added for this defect.
                  </div>
                ) : (
                  attachments.map((att) => (
                    <div
                    ...
                    >
                      <div className="flex items-center gap-3 truncate">
                        <div className="w-8 h-8 rounded-lg bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800 flex items-center justify-center shrink-0">
                        ...
                        <div className="min-w-0">
                          <a
                            ...
                            className="font-semibold text-rose-600 dark:text-rose-400 hover:underline inline-flex items-center gap-1 truncate max-w-md"
                          >
                            <span className="truncate">{att.original_name || att.file_url || att.file_path}</span>
                          ...
                          <div className="text-[10px] text-slate-400 flex items-center gap-2 truncate">
                            ...
                            {att.description && (
                              <>
                                <span>•</span>
                                <span className="italic truncate">{att.description}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      {canDevelop && (
                        <button
                          ...
                          className="text-slate-400 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors"
