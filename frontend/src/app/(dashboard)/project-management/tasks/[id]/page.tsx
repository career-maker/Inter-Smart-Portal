  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors disabled:opacity-50 cursor-pointer"
              title="Refresh Task"
            >
            ...
            {canUpdateExecution && (
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#56348f] hover:bg-[#462875] !text-white text-xs sm:text-sm font-semibold shadow-sm transition-colors cursor-pointer"
              >
                <Edit3 className="w-4 h-4 !text-white" />
                <span className="!text-white">Edit / Update Execution</span>
              </button>
            )}
            ...
            {task.project && (
              <Link
                ...
                title="Report a defect for this task in Bugzilla"
              >
            ...
            {canManage && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs sm:text-sm font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
                >
                ...
                {showDeleteConfirm ? (
                  <div className="flex items-center gap-2 p-1 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-800 rounded-xl">
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800/60 transition-colors cursor-pointer"
                    title="Delete Task"
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 p-5 sm:p-6 shadow-sm space-y-5">
            ...
            {/* QA Bug Metrics Card */}
            {((task.total_bugs ?? 0) > 0 || (task.html_bugs ?? 0) > 0 || (task.functional_bugs ?? 0) > 0 || task.bug_tracker_link) && (
              <div className="p-4 rounded-xl bg-purple-50/50 dark:bg-purple-950/20 border border-purple-200/80 dark:border-purple-800/50 space-y-3">
                <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-3 gap-2 text-center pt-1">
                  <div className="p-2 rounded-lg bg-white/80 dark:bg-slate-900/80 border border-purple-100 dark:border-purple-900/40">
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">HTML Bugs</span>
                    <span className="text-sm font-bold text-amber-600 dark:text-amber-400">{task.html_bugs || 0}</span>
                  </div>
                  <div className="p-2 rounded-lg bg-white/80 dark:bg-slate-900/80 border border-purple-100 dark:border-purple-900/40">
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">Func Bugs</span>
                    <span className="text-sm font-bold text-purple-600 dark:text-purple-400">{task.functional_bugs || 0}</span>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-100 dark:bg-purple-950 border border-purple-200 dark:border-purple-800">
                    <span className="text-[10px] text-purple-700 dark:text-purple-300 uppercase block font-bold">Total Bugs</span>
                    <span className="text-sm font-black text-purple-900 dark:text-purple-100">{task.total_bugs || 0}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="space-y-6">
                ...
                {task.assignees.map((assignee) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-purple-500/10 text-purple-600 dark:text-purple-400 font-bold text-xs flex items-center justify-center">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-slate-400">
                          {assignee.pivot?.is_primary ? "Primary Lead" : "Contributor"}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          {/* Bugzilla Project Card (Optional - Section 18 & 19) */}
          {task.project && (
            <div className="rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 p-5 shadow-sm space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  <Bug className="w-4 h-4 text-rose-600 dark:text-rose-400" />
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">Bugzilla Registry</h3>
                </div>
                <Link
                  href={`/project-management/bugzilla/bugs/new?portal_project_id=${task.project.id}&task_id=${task.id}`}
              ...
              <div className="space-y-2 text-xs">
                <div className="text-slate-600 dark:text-slate-400">
                  Defect tracking for <strong className="text-slate-800 dark:text-slate-200">{task.project.name}</strong>
                </div>
      ...
      <TaskExecutionModal
        ...
        onDelete={() => router.push("/project-management/tasks")}
      />
