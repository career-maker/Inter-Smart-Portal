    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <Link href="/issues" className="p-2 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition-colors text-slate-300 hover:text-white shrink-0">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <div className="flex items-center flex-wrap gap-3">
              <h1 className="text-xl font-bold text-white">#{issue.id} {issue.title}</h1>
        ...
        {isSuperAdmin && (
          <select
            ...
            className="bg-slate-700 border border-white/10 text-white text-sm rounded-xl px-3 py-2 outline-none focus:border-amber-500 [color-scheme:dark]"
          >
        ...
        {!isSuperAdmin && (issue.status === "Resolved" || issue.status === "Closed") && (
          <button onClick={() => handleStatusChange("Open")} className="px-4 py-2 text-sm font-medium bg-white/5 border border-white/10 text-slate-300 rounded-xl hover:bg-white/10 transition-colors">
            Reopen Issue
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-5 bg-white/5 border-b border-white/10">
              <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    <p className="font-semibold text-white">{issue.user?.first_name} {issue.user?.last_name}</p>
                    <p className="text-xs text-slate-400">{format(new Date(issue.created_at), "PPp")}</p>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-white/10">
                      {issue.attachments.map((att: any) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 p-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 text-sm text-slate-300 transition-colors"
                        >
              ...
              <div className="divide-y divide-white/5">
                {issue.comments.map((comment: any) => (
                  <div key={comment.id} className="p-5">
                    <div className="flex gap-4">
                      <div className="w-9 h-9 rounded-full bg-slate-600 flex items-center justify-center text-xs font-bold text-white shrink-0 overflow-hidden">
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-white text-sm">{comment.user?.first_name} {comment.user?.last_name}</p>
                        ...
                        <div className="text-slate-300 whitespace-pre-wrap text-sm">{comment.comment}</div>
                          ...
                          <div className="flex flex-wrap gap-2 pt-2">
                            {comment.attachments.map((att: any) => (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex items-center gap-2 p-1.5 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 text-xs text-slate-400 transition-colors"
                              >
              ...
              <form onSubmit={handleCommentSubmit} className="space-y-3">
                ...
                {files.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {files.map((f, i) => (
                      <div key={i} className="text-xs bg-white/5 border border-white/10 px-2 py-1 rounded-lg flex items-center gap-2 text-slate-300">
                ...
                <div className="flex justify-between items-center">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-amber-400 transition-colors px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10"
                  >
        ...
        <div className="space-y-4">
          <div className="bg-slate-800/80 border border-white/10 rounded-2xl p-5">
            <h3 className="font-semibold text-white mb-4 border-b border-white/10 pb-3">Details</h3>
