...
function SortableHeader({ label, col, sort, onSort }: { label: string; col: string; sort: { col: string; dir: "asc" | "desc" }; onSort: (c: string) => void }) {
  ...
  return (
    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer select-none hover:text-white transition-colors whitespace-nowrap" onClick={() => onSort(col)}>
...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <div>
        <h1 className="text-3xl font-bold text-white flex items-center gap-3"><FileText className="w-7 h-7 text-amber-400" /> Reports</h1>
        <p className="text-slate-400 mt-1">Generate, filter, and export HR reports.</p>
      </div>
      ...
      {/* Filters */}
      <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Employee</label>
        ...
        {(reportType === "leaves" || reportType === "attendance-summary") && <>
          <div>
            ...
            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
              className="bg-slate-700 border border-white/10 text-white text-sm rounded-lg px-3 py-2 outline-none focus:border-amber-500" />
          </div>
          <div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-white/10 flex items-center gap-3">
            <div className="relative flex-1 max-w-xs">
              ...
              <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search..."
                className="w-full pl-9 pr-3 py-2 bg-slate-700 border border-white/10 text-white text-sm rounded-lg outline-none focus:border-amber-500 placeholder:text-slate-500" />
            </div>
                ...
                {paginated.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    {reportType === "employees" && <>
                      ...
                      <td className="px-4 py-3 text-slate-300">{row.designation || "—"}</td>
                      <td className="px-4 py-3 text-slate-300 whitespace-nowrap">{row.team_lead || "—"}</td>
                      <td className="px-4 py-3 text-slate-300 whitespace-nowrap">{row.joining_date ? format(new Date(row.joining_date), "dd MMM yyyy") : "—"}</td>
                    ...
                    {reportType === "leave-balances" && <>
                      ...
                      <td className="px-4 py-3 text-center text-slate-300">{row.cl_used}</td>
          ...
          {totalPages > 1 && (
            <div className="p-4 border-t border-white/10 flex items-center justify-between">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 text-xs font-semibold text-slate-300 bg-white/5 rounded-lg disabled:opacity-40 hover:bg-white/10 transition">Previous</button>
              <span className="text-xs text-slate-400">Page {page} of {totalPages}</span>
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1.5 text-xs font-semibold text-slate-300 bg-white/5 rounded-lg disabled:opacity-40 hover:bg-white/10 transition">Next</button>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Users className="w-5 h-5 text-amber-400" /> Employee Profile — {emp.full_name}</h2>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        <h2 className="text-lg font-bold text-white mb-5 flex items-center gap-2"><CalendarDays className="w-5 h-5 text-emerald-400" /> Leave Information</h2>
...
function SingleLeaveBalanceReport({ emp }: { emp: any }) {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
      <h2 className="text-lg font-bold text-white mb-5 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-amber-400" /> Leave Balance — {emp.full_name}</h2>
