...
import { Gamepad2, Bug, Play, ArrowLeft, Trophy, Sparkles, ShieldAlert, Zap, Maximize2 } from "lucide-react";
import { NetworkErrorWithGame } from "@/components/ui/NetworkErrorWithGame";

export default function GamePage() {
  const [activeGame, setActiveGame] = useState<"runner" | "bugsmart" | null>(null);
    ...
    return (
      <div className="fixed inset-0 z-[99999] bg-[#06101a] w-screen h-screen overflow-hidden flex flex-col">
        ...
        <button
          ...
          className="absolute top-3 left-3 z-[100000] inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#081521]/95 hover:bg-[#0e2438] text-white font-bold text-xs border border-[#1e4460] shadow-2xl backdrop-blur-md transition-all cursor-pointer group"
  ...
  return (
    <div className="w-full max-w-6xl mx-auto py-4 sm:py-8 px-3 sm:px-6 space-y-6 sm:space-y-8">
      {/* Header Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-900 via-[#1e1b4b] to-slate-900 text-white p-6 sm:p-8 shadow-xl border border-indigo-500/20">
        <div className="relative z-10 max-w-2xl space-y-2">
          ...
          <p className="text-sm text-indigo-200/80 leading-relaxed">
            Take a quick mental reset, challenge your colleagues on the company leaderboard, or test your QA bug-spotting skills in interactive arenas.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
        ...
        {/* Game Card 2: InterSmart Pixel Runner */}
        <div className="flex flex-col justify-between rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/90 dark:border-slate-700/60 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group">
          {/* Card Preview Header */}
