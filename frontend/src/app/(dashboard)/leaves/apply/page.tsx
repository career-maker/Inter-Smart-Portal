...
import { ArrowLeft, AlertTriangle, CheckCircle, Clock, Loader2, Link2, ChevronLeft, ChevronRight, Calendar as CalendarIcon, X } from "lucide-react";
import { 
...
export default function ApplyLeavePage() {
  ...
  const inputCls = "w-full bg-white dark:bg-slate-700/50 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white text-sm rounded-xl px-3 py-2.5 outline-none focus:border-amber-500 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-colors [color-scheme:light_dark]";
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 border border-slate-200 dark:border-white/10 rounded-2xl">
          ...
          <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-5 mt-4">
            ...
            <div className="space-y-5">
              ...
              <div className="sticky bottom-0 left-0 right-0 -mx-6 -mb-6 p-4 bg-slate-100/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-white/10 z-40 flex justify-end gap-4 md:relative md:bottom-auto md:left-auto md:right-auto md:mx-0 md:mb-0 md:p-0 md:bg-transparent md:dark:bg-transparent md:backdrop-blur-none md:border-none md:z-auto md:pt-5 rounded-b-2xl">
                <button type="button" onClick={() => router.push("/leaves")} className="px-5 py-2 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-white/10 bg-white hover:bg-slate-100 dark:bg-white/5 dark:hover:bg-white/10 transition-colors">
                  Cancel
                </button>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-[99999] overflow-hidden font-sans" data-side-popup="true">
          ...
          {/* Side Drawer Panel */}
          <div
            className="fixed inset-y-0 right-0 max-w-md w-full bg-white dark:bg-slate-900 shadow-2xl flex flex-col justify-between border-l border-slate-200 dark:border-slate-800 z-[99999] animate-in slide-in-from-right duration-300 overflow-y-auto"
          ...
          >
            ...
            <div className="px-6 py-8 space-y-6 flex-1">
              ...
              <div className="flex justify-center">
                <div className="w-16 h-16 rounded-full bg-emerald-500/15 border-2 border-emerald-500/30 flex items-center justify-center animate-bounce">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-600 dark:text-slate-300 text-xs leading-relaxed">
                  Your leave request has been submitted for approval and added to your activity record.
                </p>
              </div>

              {/* Details Card */}
              <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-xl p-4 space-y-2.5 text-xs">
                ...
                <div className="flex justify-between items-center py-1 border-b border-slate-200/60 dark:border-slate-700/60">
                  ...
                  <span className="text-slate-900 dark:text-white font-semibold">
                    {startDate === endDate ? durationType : "Full Days"}
                  </span>
                </div>
              ...
              {/* Info message */}
              <p className="text-xs text-slate-600 dark:text-slate-300 bg-blue-500/10 border border-blue-500/20 rounded-xl p-3.5 leading-relaxed">
                📧 You&apos;ll receive an email notification once your request is reviewed by your Team Lead or Super Admin.
              </p>
            </div>

            {/* Footer Buttons */}
            <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 space-y-2">
              <button
                type="button"
                onClick={() => {
                  setShowSuccess(false);
                ...
                className="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl shadow-xs transition-colors cursor-pointer"
              >
                View My Leaves
              </button>
              <button
                ...
                className="w-full py-2 px-4 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-semibold rounded-xl transition-colors cursor-pointer"
              >
                Apply Another Leave
              </button>
            </div>
