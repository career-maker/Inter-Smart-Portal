...
export default function TAManagementPage() {
  ...
  return (
    ...
    >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {req.receipt_number && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
                    >
      ...
      {/* ══════════════════════════════════════════════════════════════
          MODAL 4: RECEIPT / INVOICE VOUCHER VIEW
      ══════════════════════════════════════════════════════════════ */}
      {viewVoucherItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-5">
            <div className="flex items-start justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div>
                <span className="text-[10px] font-mono uppercase font-bold text-purple-600">
                  {viewVoucherItem.receipt_number || "TA Reimbursement Receipt"}
                </span>
            ...
            <div className="p-4 rounded-xl bg-purple-50/50 dark:bg-purple-950/40 border border-purple-200/60 dark:border-purple-800/40 space-y-2 text-center">
              <span className="text-[11px] font-bold text-purple-700 dark:text-purple-300 uppercase">
                Approved Disbursement Amount
              </span>
              <div className="text-2xl font-bold text-[#56348f] dark:text-purple-300">
                ₹{formatCurrency(viewVoucherItem.approved_amount || viewVoucherItem.total_amount)}
              </div>
              {viewVoucherItem.is_paid && (
                <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800">
                  ✓ Paid & Settled ({viewVoucherItem.payment_mode || "Bank Transfer"})
                </span>
              )}
            </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                ...
                <span className="font-bold text-slate-900 dark:text-white">
                  {viewVoucherItem.user?.first_name} {viewVoucherItem.user?.last_name} ({viewVoucherItem.user?.employee_code})
                </span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-400 font-semibold">Travel Date:</span>
                <span className="font-semibold text-slate-700 dark:text-slate-300">
                  {formatDate(viewVoucherItem.date_travelled)}
                </span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-400 font-semibold">Purpose:</span>
                <span className="font-semibold text-slate-700 dark:text-slate-300">
                  {viewVoucherItem.reason}
                </span>
              </div>
              {viewVoucherItem.approval_notes && (
                <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
                  <span className="text-slate-400 font-semibold">Approval Note:</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-300">
                    {viewVoucherItem.approval_notes}
                  </span>
                </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={() => setViewVoucherItem(null)}
                className="px-4 py-2 rounded-xl bg-[#56348f] text-white text-xs font-bold"
              >
                Close
              </button>
            </div>
