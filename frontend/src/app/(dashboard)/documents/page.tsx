    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-300 mt-1">Request official documents from HR.</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl py-16 text-center space-y-3">
                  ...
                  return (
                    <tr key={req.id} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className="font-mono text-xs bg-white/10 text-slate-300 px-2 py-1 rounded-lg">
                          {req.request_number}
                        </span>
                      </td>
                      <td className="px-5 py-4 font-medium text-white">{req.subject}</td>
                      <td className="px-5 py-4 text-slate-400 text-xs">{fmtDate(req.created_at)}</td>
                      <td className="px-5 py-4">
                        {req.status === "Uploaded" ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-800 border border-white/10 rounded-2xl shadow-2xl z-10">
            <div className="px-6 py-5 border-b border-white/10">
              <h2 className="text-lg font-bold text-white">Request a Document</h2>
              <p className="text-slate-400 text-sm mt-0.5">Select the document type you need from HR.</p>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                >
                  <option value="" className="bg-slate-700 text-slate-400">Select document type...</option>
            ...
            <div className="px-6 py-4 border-t border-white/10 flex gap-3 justify-end">
              <button onClick={() => setShowRequestDialog(false)} className="px-4 py-2 text-sm text-slate-300 border border-white/10 rounded-xl hover:bg-white/5 transition-colors">
                Cancel
              </button>
              <button
      ...
      {showUploadDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-800 border border-white/10 rounded-2xl shadow-2xl z-10">
            <div className="px-6 py-5 border-b border-white/10">
              <h2 className="text-lg font-bold text-white">Fulfill Document Request</h2>
              <p className="text-slate-400 text-sm mt-0.5">
                For: <span className="text-white font-medium">{showUploadDialog?.subject}</span>
              </p>
            </div>
            <div className="px-6 py-5 space-y-5">
              {/* Mode toggle */}
              <div className="flex rounded-xl border border-white/10 overflow-hidden bg-slate-700/50 p-1 gap-1">
                <button
              ...
              {fulfillMode === "file" ? (
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-sm text-slate-300 bg-slate-700 border border-white/10 rounded-xl px-3 py-2.5
            ...
            <div className="px-6 py-4 border-t border-white/10 flex gap-3 justify-end">
              <button onClick={() => setShowUploadDialog(null)} className="px-4 py-2 text-sm text-slate-300 border border-white/10 rounded-xl hover:bg-white/5 transition-colors">
                Cancel
              </button>
              <button
