        ...
        {rawPunches.map((event, index) => {
          ...
          return (
            <div key={`${event.event_id}-${index}`} className="relative mb-6 last:mb-0">
              {/* Event dot */}
              <div
                className={`absolute -left-8 top-1 w-6 h-6 rounded-full border-2 border-slate-800 bg-slate-50 dark:bg-slate-900 flex items-center justify-center ${iconColor}`}
              >
