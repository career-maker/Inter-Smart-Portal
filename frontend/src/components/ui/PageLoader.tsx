...
const FLUX_FROM = "var(--flux-from, #56348f)";
const FLUX_TO = "var(--flux-to, #956afa)";
...
const Z_TRANSITION: Transition = { duration: 0.6, ease: [0.22, 1, 0.36, 1] };
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center text-2xl font-semibold tracking-tight text-slate-700 dark:text-slate-200 sm:text-3xl",
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative h-4 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800 shadow-[inset_0_2px_3px_rgba(0,0,0,0.09),inset_0_-1px_2px_rgba(255,255,255,0.7)] dark:shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)]",
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}
