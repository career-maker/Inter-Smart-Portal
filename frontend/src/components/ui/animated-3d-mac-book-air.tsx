...
const Macbook: React.FC<MacbookProps> = ({ className = '', scale }) => {
  ...
  // Note: macbook-key applies transform: translateZ(-2px) from globals.css
  // custom-animate-keys applies the animation.

  return (
    <div
      className={`macbook-root-wrapper ${className}`}
      style={scale ? { transform: `scale(${scale})`, transformOrigin: 'center center' } : undefined}
    >
      {/* `perspective` is applied by `macbook-container` class from globals.css */}
      <div className="macbook-container w-[150px] h-[96px] absolute left-1/2 top-1/2 mt-[-85px] ml-[-78px]">
        {/* 
          `transform-style: preserve-3d` applied by `macbook-inner` from globals.css.
        ...
        */}
        <div className="macbook-inner custom-animate-rotate z-20 absolute w-[150px] h-[96px] left-0 top-0">
          {/* Screen */}
          {/* 
            `transform-style: preserve-3d` and `transform-origin` applied by `macbook-screen` from globals.css.
          ...
          */}
          <div className={`macbook-screen custom-animate-lid-screen w-[150px] h-[96px] absolute left-0 bottom-0 rounded-[7px] bg-[#ddd] 
            ...
            {/* `transform: translateZ(2px)` applied by `macbook-screen-face-one` from globals.css */}
          ...
          {/* Body */}
          {/* 
            `transform-style: preserve-3d`, `transform-origin`, and initial `transform: rotateX(-90deg)` applied by `macbook-body` from globals.css.
          */}
          <div className={`macbook-body custom-animate-lid-macbody w-[150px] h-[96px] absolute left-0 bottom-0 rounded-[7px] bg-[#cbcbcb]
                          bg-[linear-gradient(45deg,rgba(0,0,0,0.24)_0%,rgba(0,0,0,0)_100%)]`}>
            {/* 
              `transform-style: preserve-3d` and `transform: translateZ(-2px)` applied by `macbook-body-face-one` from globals.css.
            ...
            */}
            <div className={`macbook-body-face-one custom-animate-lid-keyboard-area w-[150px] h-[96px] absolute left-0 bottom-0 rounded-[7px] bg-[#dfdfdf] 
              ...
              {/* `transform-style: preserve-3d` applied by `macbook-keyboard` from globals.css */}
        ...
        {/* 
          Initial `transform` applied by `macbook-shadow` from globals.css.
        */}
