...
export function PMAllTasksTable({
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (selectedCoordinatorId) {
      list = list.filter((t) => {
        const taskCoordId = t.coordinator?.id || t.project?.project_coordinator_id;
  ...
  return (
    <div className="rounded-2xl bg-slate-950 border border-slate-800/80 shadow-2xl overflow-hidden">
      {/* ── Table Header / Filter Bar ── */}
      <div className="p-4 sm:p-5 border-b border-slate-800/80 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/40">
        {/* Title */}
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-blue-500/10 text-blue-400 border border-blue-500/20">
            <Layers className="w-5 h-5" />
          </div>
          <h2 className="text-lg font-bold text-white tracking-tight">All Tasks</h2>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative min-w-[200px] sm:min-w-[240px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-1.5 rounded-xl bg-slate-900 border border-slate-700/80 text-white text-xs placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            />
          </div>
          ...
          <div className="relative">
            <select
              ...
              className="pl-3 pr-8 py-1.5 rounded-xl bg-slate-900 border border-slate-700/80 text-slate-300 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 cursor-pointer"
            >
          ...
          {/* Results Count Pill */}
          <div className="px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-400">
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors border ${
              showTodayOnly
