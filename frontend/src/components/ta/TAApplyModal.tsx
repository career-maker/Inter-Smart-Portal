...
export function TAApplyModal({ isOpen, onClose, onSuccess }: TAApplyModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (billLink.trim()) {
        payload.bill_link = billLink.trim();
      }
      ...
      setError(err.response?.data?.message || "Failed to submit travel allowance request. Please verify inputs.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      <div className="fixed inset-y-0 right-0 max-w-lg w-full bg-white dark:bg-slate-900 shadow-2xl flex flex-col justify-between border-l border-slate-200 dark:border-slate-800 z-50 animate-in slide-in-from-right duration-300">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="space-y-2.5">
              {items.map((item, index) => {
                const IconComponent = CATEGORY_ICONS[item.category] || Car;
                return (
                  <div
                    key={index}
          ...
          <div className="space-y-1.5">
            <label className="block text-xs sm:text-[13px] font-semibold text-slate-700 dark:text-slate-200">
              Bill / Receipt Link <span className="text-slate-400 font-normal">(Optional)</span>
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="https://drive.google.com/... or cloud bill link"
                className="w-full pl-9 pr-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/40 focus:border-purple-600 dark:focus:border-purple-500 transition-all shadow-xs"
              />
            </div>
            <p className="text-[11px] leading-[15px] text-slate-500 dark:text-slate-400">
              Provide a viewable link to your receipt on Google Drive, OneDrive, or Dropbox.
            </p>
          </div>
        </form>
